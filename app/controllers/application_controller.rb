...
class ApplicationController < ActionController::Base
  ...
  # A method to wrap exceptions thrown when redirection happens twice
  # Implemented workaround is to restrict double redirection by allowing to render the issue in a concurrent update
  def redirect_back_wrapper(default)
    back_url = params[:back_url].to_s
    if back_url.present? && valid_back_url?(back_url)
      redirect_to(:action => :back_url)
      return
    end
    begin
      redirect_to default
