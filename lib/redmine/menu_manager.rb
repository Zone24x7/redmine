  ...
  module MenuManager
    ...
    module MenuHelper
      ...
      def allowed_node?(node, user, project)
        ...
        #Disabling fields to Software Engineer- External role
        if node.name==:overview && (User.current.roles_for_project(project).include?Role.find_by_id(27))
          return false
        end
        if node.name==:news && (User.current.roles_for_project(project).include?Role.find_by_id(27))
          return false
        end
        if node.name==:roadmap && (User.current.roles_for_project(project).include?Role.find_by_id(27))
          return false
        end
        if node.name==:activity && (User.current.roles_for_project(project).include?Role.find_by_id(27))
          return false
        end
